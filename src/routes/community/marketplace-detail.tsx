  ...
  return (
    <div className="space-y-3">
      <div className="aspect-[16/10] bg-gray-100 rounded-lg overflow-hidden">
        <img src={sorted[activeIndex]?.image_file_url} alt="" className="w-full h-full object-cover" />
      </div>
      {sorted.length > 1 && (
        <div className="flex gap-2 overflow-x-auto">
          {sorted.map((img, i) => (
            ...
            >
              <img src={img.image_file_url} alt="" className="w-full h-full object-cover" />
            </button>
          ))}
        </div>
      )}
...
export function CommunityMarketplaceDetailPage() {
  const { orgSlug, communitySlug, listingId } = useParams({ strict: false }) as {
    orgSlug: string;
  ...
  return (
    <div className="max-w-3xl mx-auto p-6 pb-12 space-y-6">
      ...
      {listing.images && listing.images.length > 0 && <ImageGallery images={listing.images} />}
        ...
        {listing.expires_at && (
          <div className="bg-gray-50 rounded-lg p-3">
            ...
            <p className="text-sm font-medium text-gray-900">
              {new Date(listing.expires_at).toLocaleDateString('en-GB', { day: 'numeric', month: 'short', year: 'numeric' })}
            </p>
          </div>
        )}
      </div>
      ...
      <div className="border border-border rounded-lg p-4">
        ...
        >
          {listing.organization.image_url ? (
            <img src={listing.organization.image_url} alt={listing.organization.name} className="w-10 h-10 rounded-full object-cover" />
          ) : (
      ...
      <p className="text-xs text-gray-400">
        Posted {new Date(listing.created_at).toLocaleDateString('en-GB', { day: 'numeric', month: 'short', year: 'numeric' })}
      </p>
    </div>
  );
}
