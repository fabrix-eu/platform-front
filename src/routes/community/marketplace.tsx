...
function ListingCard({
  ...
  return (
    <Link
      to="/$orgSlug/communities/$communitySlug/marketplace/$listingId"
      params={{ orgSlug, communitySlug, listingId: listing.id }}
...
export function CommunityMarketplacePage() {
  ...
  // Find community ID from user's accessible communities
  const community = me.data?.accessible_communities?.find((c) => c.slug === communitySlug);
  const communityId = community?.id;

  const query = useQuery({
    queryKey: ['listings', 'community_members', communityId, { page, search, by_type, by_category, by_subcategory }],
    queryFn: () =>
      getListings({
        page: page || 1,
        per_page: 20,
  ...
  const basePath = `/${orgSlug}/communities/${communitySlug}/marketplace`;
        ...
        <>
          ...
          {listings.length === 0 ? (
            <div className="bg-white rounded-lg border border-border p-12 text-center space-y-3">
              ...
              <h3 className="font-display font-semibold text-gray-900">No listings yet</h3>
              <p className="text-sm text-gray-500">
                {search || by_type || by_category
                  ? 'Try adjusting your filters or search terms.'
                  : 'No community members have posted listings yet.'}
              </p>
            </div>
          ) : (
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
