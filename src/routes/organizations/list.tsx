...
export function OrganizationsListPage() {
  ...
  const mapQuery = useQuery({
    queryKey: ['organizations', 'map', { search, kinds, claimed: claimedFilter, country, lon, lat, radius }],
    queryFn: () => getOrganizations({
      per_page: 1000,
      search,
      kinds,
    ...
    <div className="flex h-[calc(100vh-56px)]">
      ...
      <div className="w-[420px] flex-shrink-0 border-r border-border bg-white flex flex-col overflow-hidden">
          ...
          <div className="flex gap-2 items-center">
            ...
            <div className="flex border border-border rounded-lg overflow-hidden">
              <button
                ...
                className={`px-2.5 py-2 text-sm ${view === 'list' ? 'bg-secondary text-secondary-foreground' : 'text-muted-foreground hover:bg-muted/50'}`}
                title="List view"
              >
                <svg className="h-4 w-4" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor">
              ...
              <button
                ...
                className={`px-2.5 py-2 text-sm border-l border-border ${view === 'cards' ? 'bg-secondary text-secondary-foreground' : 'text-muted-foreground hover:bg-muted/50'}`}
                title="Cards view"
              >
                <svg className="h-4 w-4" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor">
        ...
        <div className="flex-1 overflow-y-auto">
          {query.isLoading && <p className="p-4 text-muted-foreground">Loading...</p>}
          {query.error && <p className="p-4 text-destructive">Failed to load organizations</p>}
